...
def compute_loss(
    ...
    loss += r_tau * 0.5 * np.sum(movie_embedding * movie_embedding)
...
def update_movie_embedding(
    ...
    user_mat = user_embedding[users].T @ user_embedding[users]
    user_vec = user_embedding[users].T @ (
        ratings - user_bias[users] - movie_bias[movie_idx]
    )

    user_mat *= r_lambda
    user_mat += r_tau * np.eye(embedding_dim)
    user_vec *= r_lambda

    inv_user_mat = np.linalg.inv(user_mat)
